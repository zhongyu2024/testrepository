	...
	function init(){
	    ...
	    var tableIns = table.render({
	        ...
	        cols : [[
	            ...
	            {field: 'ins_name', title: '商户名', width:180,align:'center'},
	           ...
	           {title: '操作', width:180, 
	            	templet :function (res){
	            		if(res.is_pay==2){
	            			...
	            			return '<a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="agree">同意</a><a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="refuse">拒绝</a><a class="layui-btn layui-btn-xs" lay-event="edit">修改订单</a>'
	            		}
	                }
        ...
        if(layEvent === 'agree'){ 
        	  layer.prompt({title: '输入订单号', formType: 3}, function(text, index){
        	    layer.close(index);
  	        	  $.ajax({
			            type:"GET",
			            url:"/ins/updateInsGetMoney",
			            data:{is_pay:1,id:id,user_id:user_id,amount:amount,order_sn:text,is_check:1},
			         	datatype: "json",
			      		async:false,
  	    ...
  	    if(layEvent === 'refuse'){ 
  	      ...
  	      if(layEvent === 'edit'){ 
	        	  ...
	        	  layer.prompt({title: '输入订单号', formType: 3}, function(text, index){
  	        	  ...
  	        	  $.ajax({
			            type:"GET",
			            url:"/ins/updateVoucherOrderSn",
			            data:{id:id,order_sn:text,is_check:1},
			         	datatype: "json",
			      		async:false,
			            success:function(result){
			            	if(result==1){
