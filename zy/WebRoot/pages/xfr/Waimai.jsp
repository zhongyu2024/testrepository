...
<html>
  ...
  <!-- 点餐人数，默认为1 -->
  <input type="hidden" name="eatnum" value="1">
  <!-- 是不是下单并且派送0-不是，1-是 -->
  <input type="hidden" name="paisong" value="0" id="ppp">
    <div id="top">
    	<div id="info_left">
	    	<label>手机号码：</label><input type="text" name="tel" id="tel" placeholder="请输入手机号码" style="height:30px">
	    	<label>联系人：</label><input type="text" name="manname" id="manname" placeholder="请输入姓名" style="height:30px">
	    	<label>地址：</label><input type="text" name="address" id="address" placeholder="请输入地址" style="height:30px;width:200px">	    	
    	</div>
    	<div id="info_right">
    		<table border="0" cellpadding="0" cellspacing="5">
    			<tr>
    				<td width="95px"><img src="images/xfr/20.jpg"></td>
    				<td width="95px"><img src="images/xfr/18.jpg"></td>
    				<td width="95px"><img src="images/xfr/21.jpg"></td>
    				<td width="95px"><img src="images/xfr/22.jpg"></td>
    				<td width="95px"><img src="images/xfr/23.jpg"></td>
    			</tr>
    		</table>
    	</div>
    	</div>
  ...
  var w=$(window).width()+"px";
  	$(function(){
		...
		danhao="W"+year+month+date+h+m+s;
		$("#danhao").html(danhao);
		//对当前清单的操作(作废)
		$("#info_right").find("td:eq(4)").click(function(){
			save(3);
		});
		//保存
		...
		$("#info_right").find("td:eq(3)").click(function(){
			window.location="Waimai/showquhui.do?pageNum=1&t=1";
		});
		//结账
  	...
  	$("#subbtn").click(function(){
  		var ss=$("<input type='hidden' name='sendman' value='"+$("#pai").val()+"'>");
  		ss.appendTo($("#ifm"));
  		$("#ppp").val("1");
  		$("#ifm").attr("action","Waimai/zuofei.do");
		$("#sss").val("4");
		$("#ifm").submit();
  		$("#cover").css({"display":"none"});
  	...
  	function save(index){
  		if($("#msg_1_top").next().html()==null){
				...
				if(checkinfo()){
					if(index==1){
						//结账
  						$("#ifm").attr("action","Waimai/jiezhang.do");
						$("#sss").val("4");
			  		}else if(index==2){
			  			//保存
			  			$("#ifm").attr("action","Waimai/zuofei.do");
